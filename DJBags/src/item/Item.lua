...
function item:Update()
    ...
    if bag == BANK_CONTAINER or bag == REAGENTBANK_CONTAINER then
        ...
        local isQuestItem, questId, isActive = C_Container.GetContainerItemQuestInfo(bag, self:GetID())
        local isNewItem = C_NewItems.IsNewItem(bag, self:GetID())
        local isBattlePayItem = C_Container.IsBattlePayItem(bag, self:GetID())
...
function item:UpdateSearch()
    local _, _, _, _, _, _, _, filtered, _, id = C_Container.GetContainerItemInfo(self:GetParent():GetID(), self:GetID())
    self:SetFiltered(filtered)
end

function item:UpdateLock(locked)
    local locked = locked or select(3, C_Container.GetContainerItemInfo(self:GetParent():GetID(), self:GetID()))
    SetItemButtonDesaturated(self, locked);
end
